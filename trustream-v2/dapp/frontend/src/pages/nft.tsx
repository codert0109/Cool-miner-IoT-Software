...
export default observer(() => {
  ...
  const onBuyNFT = async (type_id: number) => {
    if (nft.loading) {
      Swal.fire('Error!', 'Cannot Buy NFT while loading information.', 'error');
      return;
    }
    ...
    try {
      ...
      if (receipt.status == 0) {
        ...
        await receipt.wait();
        Swal.fire('Awesome!', 'Your NFT purchase has been completed!', 'success');
        nft.refresh();
      }
    } catch (error) {
      const ERROR_CODE_TX_REJECTED_BY_USER = 4001;
      if (error.code === ERROR_CODE_TX_REJECTED_BY_USER) {
        Swal.fire('Error!', 'You rejected transaction.', 'error');
        return;
      } else {
  ...
  const onClaimTokens = () => {
    ...
    if (hasBalance()) {
      Swal.fire('Error!', 'You can not claim tokens since you already buy an NFT.', 'error');
      return;
    }
  ...
  const renderOWNEDNFT = () => {
    if (hasNFT() == false) {
      return <div className={classes.infodiv + ' ' + classes.warning}>You need to buy an NFT in order to mine.</div>;
    }
      ...
      <>
        ...
        <SimpleGrid cols={3} breakpoints={[{ maxWidth: 'xs', cols: 1 }]}>
          {nft.infoList.map((item, index) => {
            return <NFTStatus title="Testnet Miner" imgurl="/images/nft/TestNet.png" price={nft.typeList[item.nftType].price + ' IOTX'} acquiredTime={item.acquireTime} id={nft.idList[index]} />;
          })}
        </SimpleGrid>
      </>
    );
      ...
      {!nft.loading && nft.typeList.length > 0 && (
        <>
          ...
          <SimpleGrid cols={3} breakpoints={[{ maxWidth: 'xs', cols: 1 }]}>
            ...
            />
            <NFTMinerNode title="Public Pool Miner - mainnet" imgurl="/images/nft/PublicPool.png" price="xx" comment="Qty available Limited" text="COMING SOON! LIMITED AVAILABILITY" disabled={true} />
            <NFTMinerNode title="Webcam Miner - Phase 2" imgurl="/images/nft/Webcam.png" price="xx" comment="Qty available xx" text="COMING in Phase 2!" disabled={true} />
          </SimpleGrid>
        </>
      )}
    </Layout>
